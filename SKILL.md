...
- 所有可見教材與介面文字在各必要 viewport 的計算後字級不得低於 `16px`；放大後若擁擠，重構欄寬、間距或版面，不得縮回小字。
...
3. 檢查頁面內容、錯誤覆蓋層、主控台、失敗請求、水平溢出、裁切、文字重疊、引線、動畫前中後狀態、控制元件、鍵盤與降低動畫模式；逐頁掃描可見文字的計算後字級，低於 `16px` 的元素數必須為 0。
...
- 任一必要 viewport 或主要互動狀態仍有計算後字級低於 `16px` 的可見文字。
